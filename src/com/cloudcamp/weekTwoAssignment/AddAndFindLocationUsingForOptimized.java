...
public class AddAndFindLocationUsingForOptimized {
  ...
  public static void findAClinic(int pinCode){
    ...
    for (int currentPosition = 0; currentPosition < size; currentPosition++) { // we are not looping through entire array.
      if (clinics[currentPosition].pinCode == pinCode) {
        clinicFound = true;
        clinics[currentPosition].displayClinic();
      }
    }
